...
- Production serving should use two database tiers on separate VMs: an internal patient-linked
  database and a public/sanitized variant database populated only by an allowlisted export.
...
- Calculate cohort-level `ac_total`, `an_total`, `af_total`, and `hom_count` from Hail
  `variant_qc()`.
- Public exports must never contain patient IDs, `sample_id`, per-sample genotypes, clinical
  metadata, dates, HPO assignments, report flags, run IDs, care sites, or internal paths.
